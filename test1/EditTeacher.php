<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        ...
        if ($result->num_rows == 0) {
            echo "No records found for TeacherID: " . $TeacherID;
            exit;
        }
        ...
        if ($row !== null) {
            ...
            $Teacher_PhoneNumber = $row["Teacher_PhoneNumber"];
            $Teacher_DateOfBirth = $row["Teacher_DateOfBirth"];
            $Teacher_AnnualSalary = $row["Teacher_AnnualSalary"];
            $Teacher_BackgroundCheck = $row["Teacher_BackgroundCheck"];
            $ClassID = $row["ClassID"];
        }
        ...
        if (empty($TeacherID) || empty($Teacher_Name) || empty($Teacher_Address) || empty($Teacher_PhoneNumber) || empty($Teacher_DateOfBirth) || empty($Teacher_Email) || empty($Teacher_AnnualSalary) || empty($Teacher_BackgroundCheck)) {
            $errorMessage = "All fields are required";
        } else {
            // Check for numeric values
            if (!is_numeric($TeacherID)) {
                $errorMessage = "Invalid TeacherID";
            } else {
                ...
                if (empty($errorMessage)) {
                    ...
                    if (!$result) {
                        ...
                        if ($Teacher_Count > 0) {
                            // Check if the teacher to be updated belongs to the selected TeacherID and ClassID
                            $CheckTeacherSQL = "SELECT TeacherID FROM teacher WHERE ClassID = ? AND TeacherID = ?";
                            $stmt = $connection->prepare($CheckTeacherSQL);
                            $stmt->bind_param("ii", $ClassID, $TeacherID);
                            $stmt->execute();
                            $result = $stmt->get_result();
    
                            if (!$result) {
                                $errorMessage = "Error checking teacher: " . $stmt->error;
                            } else {
                                if ($result->num_rows === 0) {
                                    $errorMessage = "The teacher does not belong to the selected class.";
                                } else {
                                    // Proceed with the update
                                    $UpdateTeacherSQL = "UPDATE teacher SET Teacher_Name=?, Teacher_Address=?, Teacher_Email=?, Teacher_PhoneNumber=?, Teacher_DateOfBirth=?, Teacher_AnnualSalary=?, Teacher_BackgroundCheck=? WHERE TeacherID=?";
                                    $stmt = $connection->prepare($UpdateTeacherSQL);
                                    $stmt->bind_param("sssssssi", $Teacher_Name, $Teacher_Address, $Teacher_Email, $Teacher_PhoneNumber, $Teacher_DateOfBirth, $Teacher_AnnualSalary, $Teacher_BackgroundCheck, $TeacherID);
                                    $stmt->execute();
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                ?>
                    ...
                    <form class="Teacher-form row" method="POST" action="EditTeacher.php" id="TeacherForm">
